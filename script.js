...
milkyWayGeometry.setAttribute(
  "position",
  new THREE.Float32BufferAttribute(milkyWayPositions, 3)
);
...
function animatePlanet(planet) {
  ...
  gsap.to(planet.orbitElement, {
    ...
    onUpdate: function () {
      ...
      planet.orbitElement.style.left = `${Math.min(
        Math.max(newX, minX),
        maxX
      )}px`;
      planet.orbitElement.style.top = `${Math.min(
        Math.max(newY, minY),
        maxY
      )}px`;
      angle += planet.speed;
...
function startCorrectingAnimation() {
  ...
  const correctingInterval = setInterval(() => {
    if (i < totalLength) {
      let correctedText =
        currentText.slice(0, i) + fullText[i] + currentText.slice(i + 1);
...
function animateText(elementId, text, delay = 0) {
  ...
  gsap.fromTo(
    ...
    { textContent: "" },
    {
      ...
      stagger: 0.1,
      onUpdate: function () {
        element.innerText = text.substring(
          0,
          Math.ceil(this.progress() * text.length)
        );
      },
    }
  );
